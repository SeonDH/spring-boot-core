...
public class Factory {
    ...
    private static <T> List<Class<? extends T>> scanDirectoryForAnnotatedClasses(String packageName, Class<?> interfaceName) {
        ...
        URL resource = java.util.Objects.requireNonNull(classLoader.getResource(path));

        File directory;
        try {
            URI uri = resource.toURI(); // URL -> URI (한글 대응)
            directory = Paths.get(uri).toFile();
        } catch (Exception e) {
            throw new RuntimeException("리소스를 URI로 변환할 수 없습니다: " + resource, e);
        }
        ...
        annotatedClasses = annotatedClasses.stream()
                .filter(it -> it.isAnnotationPresent(Part.class) || it.isAnnotationPresent(First.class))
                .toList();

        if (annotatedClasses.size() > 1) {
            annotatedClasses = annotatedClasses.stream()
                    .filter(it -> it.isAnnotationPresent(First.class))
                    .toList();
        }
